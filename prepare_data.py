...
class VideoFramesDataset(Dataset):
    ...
    def _setup_data(self):
        ...
        for video, n_frame, label in zip(video_names, num_frames, label_idxs):
            # Read frames
            frame_paths = sorted(
                glob.glob(os.path.join(self.root_dir, video, '*')))
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Prepare data for Video Classification')

    # Dataset params
    parser.add_argument('-f',   '--frames_dir',     type=str,
                        help='input frames root directory', required=True)
    parser.add_argument('-j',   '--json_file',      type=str,
                        help='input train/val/test json file', required=True)
    parser.add_argument('-o',   '--out_dir',        type=str,
                        help='stores processed json & embeddings npy', required=True)

    # Model params
    parser.add_argument('-m',   '--model',          type=str,
                        help='pre-trained CNN (torchvision.models)', choices=CNN_NAMES)
    parser.add_argument('-s',   '--split',          type=str,
                        help='select split', choices=['train', 'val', 'test'], required=True)
    parser.add_argument('-bs',  '--batch_size',     type=int,
                        help='batch size for computing embeddings', default=128)

    # Misc params
    parser.add_argument('-nw',  '--num_workers',    type=int,
                        help='no. of worker threads for dataloader', default=1)
    parser.add_argument('-g',   '--gpu_id',         type=int,
                        help='cuda:gpu_id (torch.device)', default=0)
    ...
    df['video_length'] = df['video_name'].apply(
        lambda x: _count_frames(x, args.frames_dir))

    # Image Mean & Std-Dev for Normalization
    mean = np.array([0.485, 0.456, 0.406])
    std = np.array([0.229, 0.224, 0.225])

    dataset = VideoFramesDataset(args.frames_dir, df, Compose(
    ...
    embeddings_temp = np.memmap(
        emb_temp_file, 'float32', 'w+', shape=(total_frames, emb_dim))
    ...
    with torch.no_grad(), tqdm(desc='Embeddings', total=len(dataloader)) as progress_bar:
        ...
        for batch in dataloader:
            ...
            # [batch_size, emb_dim]
            emb = model(frames).cpu().detach().numpy()
    ...
    embeddings_final = np.memmap(
        embeddings_file, 'float32', 'w+', shape=memmap_shape)

    j = 0
    for video_idx, video_len in enumerate(video_lengths):
        embeddings_final[video_idx, :video_len,
                         :] = embeddings_temp[j: j+video_len, :]
